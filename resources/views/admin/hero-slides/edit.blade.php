                        ...
                        <div class="p-6">
                            <div class="relative aspect-video w-full overflow-hidden rounded-xl bg-gray-900"
                                ...
                                @else
                                    <div class="absolute inset-0 flex items-center justify-center text-gray-400">
                                        <div class="text-center">
                                            ...
                                            <p class="text-sm">Current slide preview</p>
                                        </div>
                                    </div>
                                @endif
                                ...
                                <div id="contentPreview"
                                    class="absolute inset-0 flex items-center p-8 {{ $heroSlide->has_content ? '' : 'hidden' }}">
                                    <div class="max-w-2xl {{ $heroSlide->content_position === 'center'
                                        ? 'text-center'
                                        : ($heroSlide->content_position === 'right'
                                            ? 'text-right'
                                            : 'text-left') }}"
                                        id="previewContent">
                            ...
                            <div class="mt-4 space-y-3">
                                <div class="flex items-center justify-between">
                                    ...
                                    <span id="previewPosition"
                                        class="text-sm text-gray-600">{{ ucfirst($heroSlide->content_position) }}</span>
                                </div>
                                <div class="flex items-center justify-between">
                                    <span class="text-sm font-medium text-gray-700">Has Content:</span>
                                    <span id="previewHasContent"
                                        class="text-sm text-gray-600">{{ $heroSlide->has_content ? 'Yes' : 'No' }}</span>
                                </div>
                                <div class="flex items-center justify-between">
                                    <span class="text-sm font-medium text-gray-700">Has CTA:</span>
                                    <span id="previewHasCta"
                                        class="text-sm text-gray-600">{{ $heroSlide->has_cta ? 'Yes' : 'No' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
...
@push('scripts')
    <script>
        ...
        function updatePreview() {
            ...
            const contentPosition = document.querySelector('input[name="content_position"]:checked')?.value ||
                '{{ old('content_position', $heroSlide->content_position) }}';
            ...
            document.getElementById('previewPosition').textContent = contentPosition ? contentPosition.charAt(0)
                .toUpperCase() + contentPosition.slice(1) : 'Left';
            ...
            if (contentPosition === 'left' || !contentPosition) {
            ...
            // Update content
            const badge = document.getElementById('badge_en')?.value || '{{ old('badge_en', $heroSlide->badge_en) }}';
            const title = document.getElementById('title_en')?.value || '{{ old('title_en', $heroSlide->title_en) }}' ||
                'Sample Title';
            const subtitle = document.getElementById('subtitle_en')?.value ||
                '{{ old('subtitle_en', $heroSlide->subtitle_en) }}' || 'Sample subtitle text that describes your offer';
