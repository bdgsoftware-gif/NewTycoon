<div class="w-full py-12 max-w-8xl mx-auto px-4 relative category-section">
    <h2 class="category-heading text-4xl font-bold text-gray-900 mb-10 leading-tight capitalize font-quantico">
        ...
        @if ($categories->isNotEmpty())
            @foreach ($categories as $item)
                <a href="{{ route('categories.show', $item->slug) }}"
                    ...
                    <div
                        class="relative bg-white rounded-xl p-0.5 shadow-sm flex flex-col h-40 overflow-hidden category-tilt-inner">
                        ...
                        <div class="relative flex flex-col h-full p-4 bg-white rounded-[10px] z-10">
                            ...
                            <div class="flex-grow flex items-center justify-center relative">
                                <img src="{{ asset('storage/' . $item->image) }}" alt="{{ $item->name }}"
                                    class="category-image h-16 md:h-24 object-contain transition-all duration-500 group-hover:scale-110 relative z-20">
                            </div>
