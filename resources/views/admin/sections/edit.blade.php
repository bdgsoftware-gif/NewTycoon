...
@section('content')
    <div class="max-w-3xl mx-auto">
        <form action="{{ route('admin.content.sections.update', $section) }}" data-form method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-lg shadow p-6 space-y-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Internal Name <span
                                class="text-primary">*</span></label>
                        <input type="text" name="name" id="name" value="{{ old('name', $section->name) }}"
                            class="w-full rounded-lg border-gray-300 focus:border-primary focus:ring-primary"
                            placeholder="Update Internal Name in English">
                        @error('name')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        <label for="title_en" class="block text-sm font-medium text-gray-700 mb-1">Display Title <span
                                class="text-primary">*</span></label>
                        <input type="text" name="title_en" id="title_en"
                            value="{{ old('title_en', $section->title_en) }}"
                            class="w-full rounded-lg border-gray-300 focus:border-primary focus:ring-primary"
                            placeholder="Update Display Title in English">
                        @error('title_en')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="text" name="title_bn" id="title_bn"
                            value="{{ old('title_bn', $section->title_bn) }}"
                            class="w-full rounded-lg border-gray-300 focus:border-primary focus:ring-primary"
                    ...
                    <div>
                        <label for="type" class="block text-sm font-medium text-gray-700 mb-1">Section Type <span
                                class="text-primary">*</span></label>
                        <select name="type" id="type"
                            class="w-full rounded-lg border-gray-300 focus:border-primary focus:ring-primary">
                            <option value="product_slider"
                                {{ old('type', $section->type) == 'product_slider' ? 'selected' : '' }}>Product Slider
                            </option>
                            <option value="banner" {{ old('type', $section->type) == 'banner' ? 'selected' : '' }}>Banner
                                Section</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        <label for="ad_banner_id" class="block text-sm font-medium text-gray-700 mb-1">Associated
                            Banner <span class="text-primary">*</span></label>
                        <select name="ad_banner_id" id="ad_banner_id"
                            class="w-full rounded-lg border-gray-300 focus:border-primary focus:ring-primary">
                            <option value="">None (No Banner)</option>
                            @foreach ($banners as $banner)
                                <option value="{{ $banner->id }}"
                                    {{ old('ad_banner_id', $section->ad_banner_id) == $banner->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <textarea name="settings" id="settings" rows="4"
                        class="w-full rounded-lg border-gray-300 focus:border-primary focus:ring-primary font-mono text-sm">{{ old('settings', json_encode($section->settings, JSON_PRETTY_PRINT)) }}</textarea>
                    @error('settings')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
